    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="p-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold flex items-center gap-2">
                ...
                Network Peers ({{ peers.value.size }})
            </h2>
        </div>
        <div class="p-4">
            <div v-if="peers.value.size === 1"
                 class="text-center text-gray-500 py-4">
                No other peers connected
            </div>
            <div v-else
                 class="space-y-2">
                <div v-for="peer in Array.from(peers.value)"
                     :key="peer"
                     class="flex items-center gap-2 p-2 rounded"
                     :class="peer === nodeId ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-gray-50 dark:bg-gray-700'">
                    <div class="w-2 h-2 rounded-full bg-green-500"></div>
                    <span class="text-sm font-mono">{{ peer.slice(0, 20) }}...</span>
                    <span v-if="peer === nodeId"
